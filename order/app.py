...
async def checkout(request: Request):
    ...
    if result.get("status") == "success":
        ...
        # Delete idempotency key on failure so the order can be retried
        # immediately after conditions improve (stock/credit added).
        # The "processing" claim (SET NX in order_load_and_claim) prevents
        # concurrent checkouts while a saga is in-flight; once the saga
        # completes, cleanup allows fresh retries.
        await db.delete(idempotency_key)
...
async def dlq_status(request: Request):
    ...
    count = await db.xlen("{order-wal}:dlq:saga")
    raw_entries = await db.xrevrange("{order-wal}:dlq:saga", count=10)
    entries = []
